...
Given('une tâche {string} existe', async function (texte: string) {
...
When("j'ajoute une tâche {string}", async function (texte: string) {
  await this.page.getByLabel('Nouvelle tâche').fill(texte)
})

When("j'appuie sur le bouton {string}", async function (label: string) {
  await this.page.getByRole('button', { name: label }).click()
})

When('je supprime la tâche {string}', async function (texte: string) {
  const item = this.page.locator('li.todo-item', { hasText: texte })
  await item.getByRole('button', { name: 'Supprimer' }).click()
})

When('je coche la tâche {string}', async function (texte: string) {
  const item = this.page.locator('li.todo-item', { hasText: texte })
  await item.getByRole('button', { name: 'Marquer comme terminée' }).click()
})

When('je filtre sur {string}', async function (filtreLabel: string) {
  await this.page.getByRole('tab', { name: filtreLabel }).click()
})

Then('la tâche {string} est ajoutée', async function (texte: string) {
  await expect(this.page.getByText(texte, { exact: true })).toBeVisible()
})

Then("la tâche {string} n'est plus visible", async function (texte: string) {
  await expect(this.page.getByText(texte, { exact: true })).not.toBeVisible()
})

Then('la tâche {string} est visible', async function (texte: string) {
  await expect(this.page.getByText(texte, { exact: true })).toBeVisible()
})

Then('la tâche {string} est marquée comme terminée', async function (texte: string) {
  const item = this.page.locator('li.todo-item', { hasText: texte })
